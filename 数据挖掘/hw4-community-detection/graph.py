...
class WeightedDiGraph:
    ...
    def __init__(self, edge_list: List[Tuple[int, int, int]]):
        ...
        # Mapping: node_id -> neighbors going into the node
        self.in_neighbors: Dict[int, Set[int]] = defaultdict(set)
        # Mapping: node_id -> neighbors going out of the node
        self.out_neighbors: Dict[int, Set[int]] = defaultdict(set)
        # Mapping: (src, dst) -> edge weight
        self.edges: Dict[Tuple[int, int], float] = defaultdict(float)

        # Mapping: node_id -> (weighted) in-degree of the node
        self.in_degree: Dict[int, int] = defaultdict(int)
        # Mapping: node_id -> (weighted) out-degree of the node
    ...
    def add_edge(self, src: int, dst: int, weight: int = 1):
        ...
        This function also updates the in-weights and out-weights of the nodes.
...
if __name__ == "__main__":
    g = WeightedDiGraph.from_csv_edges("./data/test_graph.csv")
